...
#define INPUTCOUNT 3	 //** number of inputs, 0 to 4. If 0, comment out the next line.
const char* const inputName[INPUTCOUNT] = {"MPLAYER",  "OPT DAC",  "USB DAC"}; //** each name maximum 9 characters. There must be exactly INPUTCOUNT names in the list.

#define OUTPUTCOUNT 1  //** number of outputs, 0 to 4. If 0, comment out the next line.
const char* const outputName[OUTPUTCOUNT] = { "LINE"}; //** each name maximum 9 characters. There must be exactly OUTPUTCOUNT names in the list.

char msgWelcome1[] = "ZDR 3x1";                     //** <-- maximum 20 characters (line 1)
...
#define IMPEDANCESTEP 5       //** increment for impedance steps in K
...
#define logoFont X11fixed7x14
#define RemoteIRTune 25000    //** Adjust this value to tune remote control operation (Default 25000)
...
#define LDR_LSE_MIN 101        //** measured value of left series LDR R at maximum current
#define LDR_LSH_MIN 138       //** measured value of left shunt LDR R at maximum current
...
// gainclone
#elif PCB == 6
#define LDR_VOLTAGE 4.93       //** precisely measured value of the +5V supply (with decimal point. Default: 5.0)
#define LDR_LSE_MIN 83        //** measured value of left series LDR R at maximum current
...
#define LDR_RSH_MIN 130       //** measured value of right shunt LDR R at maximum current

// CGSONG
#elif PCB == 0
#define LDR_VOLTAGE 4.95      //** precisely measured value of the +5V supply (with decimal point. Default: 5.0)
#define LDR_LSE_MIN 131        //** measured value of left series LDR R at maximum current 82
#define LDR_LSH_MIN 89       //** measured value of left shunt LDR R at maximum current  131
#define LDR_RSE_MIN 74       //** measured value of right series LDR R at maximum current 131
#define LDR_RSH_MIN 88        //** measured value of right shunt LDR R at maximum current  80
...
#define TIME_SWITCHBOUNCE 150       // Time in millisec to debounce switch
...
#ifndef DEBUG // turn calibration part of if debugging is on
  calibStarted = millis();
  //** local vars
  float att;
...
inline void setLSH_Range(byte val) {                   ///tagme
...
int getIRkey() {
  ...
  while ((duration = pulseIn(PIN_REMOTE, HIGH, RemoteIRTune)) < 2000 && duration != 0)
  {
  ...
  if (duration > 1000) {          // This is the repeat
    PRINT("repeat\n");
    repeat = true;
  }
...
void saveIOValues() {
  ...
  if (EEPROM.read(6) != impedance)
    EEPROM.write(6, impedance);
}
...
void setup() {
  ...
  oled.begin(&OLED_TYPE, I2C_ADDRESS);
